...
const HitDistribution: React.FC = observer(() => {
  ...
  const hitDist = thisLoadoutResult?.hitDist || [];
  const specHitDist = thisLoadoutResult?.specHitDist || [];
  const specAvailable = specHitDist.length > 0;
    ...
    >
      <div className="px-6 py-4">
        ...
        <div className="flex flex-col gap-6">
          <HitDistributionChart data={hitDist} title="Regular attack" />
          {specAvailable && (
            <HitDistributionChart data={specHitDist} title="Special attack" />
          )}
        </div>
      </div>
    </SectionAccordion>

...
const EquipmentPresets: React.FC<EquipmentPresetsProps> = ({ side }) => {
  ...
  type HeaderItem = { header: true, label: string };
  type ItemType = typeof filteredPresets[number] | HeaderItem;
    ...
    switch (v.value) {
      ...
      case EquipmentPreset.ARMADYL_GODSWORD_SPEC: {
        newPlayer = {
          name: v.label,
          equipment: {
            ammo: findItemById(21932) ?? null, body: findItemById(4736) ?? null, cape: findItemById(21295) ?? null, feet: findItemById(29806) ?? null, hands: findItemById(7462) ?? null, head: findItemById(12931) ?? null, legs: findItemById(4738) ?? null, neck: findItemById(6585) ?? null, ring: findItemById(25975) ?? null, shield: null, weapon: findItemById(11802) ?? null,
          },
        }; break; }
      // Defender presets
      ...
      case EquipmentPreset.CRYSTAL_TANK: {
        newPlayer = {
          name: v.label,
          equipment: {
            ammo: findItemById(21932) ?? null, body: findItemById(23975) ?? null, cape: findItemById(21295) ?? null, feet: findItemById(29806) ?? null, hands: findItemById(7462) ?? null, head: findItemById(24271) ?? null, legs: findItemById(23979) ?? null, neck: findItemById(6585) ?? null, ring: findItemById(25975) ?? null, shield: findItemById(11283) ?? null, weapon: findItemById(29594) ?? null,
          },
        }; break; }
  ...
  const styleOptions: StyleFilter[] = side === 'attacker'
    ? ['ranged', 'magic', 'melee', 'spec', 'all']
    : ['tank', 'robes'];

  const formatCategory = (c: typeof categories[number]) => {
    if (c === 'all') return 'All';
    if (c === 'medMax') return 'Med/Max';
    return c.charAt(0).toUpperCase() + c.slice(1);
  };

  const formatStyle = (s: StyleFilter) => {
    if (s === 'all') return 'All';
